...
/**
 * An activity representing a list of Documents. This activity has different presentations for
 * handset and tablet-size devices. On handsets, the activity presents a list of items, which when
 * touched, lead to a {@link DocumentDetailActivity} representing item details. On tablets, the
 * activity presents the list of items and item details side-by-side using two vertical panes.
 *
 * <p>The activity makes heavy use of fragments. The list of items is a {@link DocumentListFragment}
 * and the item details (if present) is a {@link DocumentDetailFragment}.
 *
 * <p>This activity also implements the required {@link DocumentListFragment.Callbacks} interface to
...
public class DocumentListActivity extends Activity implements DocumentListFragment.Callbacks {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // TODO: enable this with POI 4.0.0 as only then we have CTSignedTwipsMeasure:
    ...
    if (findViewById(R.id.document_detail_container) != null) {
      ...
      ((DocumentListFragment) getFragmentManager().findFragmentById(R.id.document_list))
  ...
  private void writeWorkbook() throws java.io.IOException {
    ...
    try {
      ...
      style.setFillBackgroundColor(new XSSFColor(new net.windward.android.awt.Color(1, 2, 3)));
  ...
  /**
   * Callback method from {@link DocumentListFragment.Callbacks} indicating that the item with the
